...
> **บทนี้เตรียมอะไร:** เขียน Prompt ถามความช่วยเหลือ AI ได้อย่างมีประสิทธิภาพ — ทักษะนี้ใช้ตลอดทั้ง course ทุกครั้งที่ติด Error
...
**Prompt Engineering** คือทักษะการเขียนคำสั่งให้ AI เข้าใจความต้องการที่แท้จริงได้อย่างชัดเจน AI ไม่สามารถ "เดาใจ" หรือดูสภาพแวดล้อมรอบตัวได้ — ถ้าให้คำสั่งคลุมเครือ จะได้คำตอบที่นำไปใช้ไม่ได้
...
### ขั้นตอนที่ 1 — เปรียบเทียบ Prompt แย่ vs ดี
...
### ขั้นตอนที่ 2 — วิธีถาม AI เมื่อเจอ TypeScript Error
...
**สรุปการทำงาน:** Context → Stack → Error message (copy ทั้งหมด) → โค้ดที่เกี่ยวข้อง → สิ่งที่ต้องการ
...
- [ ] เข้าใจว่า Prompt ที่ดีต้องมีกี่ส่วนและส่วนไหนบ้างแล้ว
- [ ] มี ChatGPT, Claude, หรือ Gemini เปิดพร้อมใช้งานแล้ว
:::
...
### 📝 PjBL Lab — ชิ้นงาน: บันทึก Prompt ที่ดีที่สุด 3 รายการ
...
- [ ] `git add . && git commit -m "wk1-content3: ai prompt by ชื่อ-นามสกุล" && git push`
- [ ] Google Doc: แปะ screenshot Prompt ที่ได้ผลดีที่สุด + สรุป 3-5 บรรทัด ✅
...
| Mistake | ผลลัพธ์ | วิธีแก้ |
| :--- | :--- | :--- |
| ถาม Prompt แบบกว้างเกินไป | ได้คำตอบที่ใช้ไม่ได้จริง | ใส่ Stack + Context + Problem ให้ครบ 4 ส่วน |
| เริ่ม Conversation ใหม่ทุกครั้ง | AI ไม่รู้ context เดิม | ใช้การ iterate ในการสนทนาเดิม |
| Copy code โดยไม่ทดสอบ | Code อาจไม่ทำงานหรือมี bug ซ่อนอยู่ | ทดสอบโค้ดทุกครั้งก่อนใช้งานจริง |
...
| :--- | :--- | :--- |
| `Prompt` | พรอมต์ | คำสั่ง/คำถามที่ส่งให้ AI |
| `Context` | คอน-เทกซ์ท | บริบท — ข้อมูลพื้นหลังที่ช่วยให้ AI เข้าใจสถานการณ์ |
| `Stack` | สแตค | กลุ่ม Technology ที่ใช้ในโปรเจกต์ทั้งหมด |
| `Output` | เอ้า-พุต | ผลลัพธ์ที่ต้องการได้รับจาก AI |
| `Iteration` | อิท-เทอ-เร-ชัน | การปรับปรุงซ้ำ — แก้ Prompt และลองใหม่เรื่อย ๆ |
| `Hallucination` | ฮัล-ลู-ซิ-เน-ชัน | AI ตอบข้อมูลผิดแต่ฟังดูน่าเชื่อ — ต้องตรวจสอบเสมอ |
